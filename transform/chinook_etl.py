...
def join_tracks(items :list):
    ...
    df = tracks_data.merge(albums_data, on='AlbumId', how='left', suffixes=('_tracks', '_albums')).merge(media_types_data, on='MediaTypeId', how='left', suffixes=('_albums', '_media_types')).merge(genres_data, on='GenreId', how='left', suffixes=('_media_types', '_genres')).merge(artists_data, on='ArtistId', how='left', suffixes=('_genres', '_artists')).drop(['AlbumId', 'MediaTypeId', 'GenreId', 'ArtistId'], axis=1)
